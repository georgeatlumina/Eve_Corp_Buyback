...
| [module-sorter.js](renderer/module-sorter.js) | ~240 | **Module Sorter tab** (General group) — self-contained IIFE loaded after `app.js` (reuses `$`/`escapeHtml`/`downloadBlob` and reads `readinessState`/`scanAllFits`/`activateTab` as cross-script globals). Splits a pasted EVE inventory / multibuy list into **doctrine** vs **non-doctrine** panes (each copy + download as multibuy). The doctrine set is the union of every item across every fit in the Market Readiness scan (`readinessState.scan`, localStorage), matched by normalised name; parses tab-separated inventory rows, `Name xN` multibuy, and bare names; sums duplicate stacks. An "exclude hulls" toggle reclassifies doctrine ship hulls. **Auto-triggers `scanAllFits()` in the background** the first time the tab is opened with no scan present (once per session; skips if a scan exists or one is already running). Lazy-loads on first tab click. |
...
| [pi.js](renderer/pi.js) | ~190 | **PI Planner tab** (General group) — self-contained IIFE after `app.js`. Search a solar system → its planet types / extractable P0 → the most profitable P0→P4 chains buildable there, priced at Jita with a configurable POCO tax; a draggable-free recipe popup shows a chain's full tree + raw-P0 basket. Backed by `/api/pi/data` + `/api/pi/analyze`. |
| [pi-builder.js](renderer/pi-builder.js) | ~450 | **PI Builder tab** (General group). Renders a planet as a rotatable/zoomable orthographic sphere (3-D vector projection); places command centers / extractors / factories / storage / launchpads at real lat/lon, links them, assigns factory schematics + extractor resources + heads, and edits routes. Live CPU/Powergrid budget (pin loads + extractor heads + level-0 link cost `15+0.2·km` / `10+0.15·km`). Loads colonies from & saves importable templates to the EVE folder via `/api/pi/templates*`; `window.piBuilderLoad` accepts a live colony pushed from PI Colonies. |
| [pi-colonies.js](renderer/pi-colonies.js) | ~120 | **PI Colonies tab** (Planetary group) — live colonies across authed characters (`/api/pi/colonies`) with 1 s extractor-expiry countdowns, per-colony status (expired / <24h / active / idle) + alert banner, sorted by soonest expiry. "Open in Builder" pulls a colony onto the builder canvas (`/api/pi/colony`). |
| [fitting.js](renderer/fitting.js) | ~700 | **Fitting tab** (Combat group) — a ship fitter on Pyfa's `eos` engine (backend `/api/fit/*`). Self-contained IIFE after `app.js` (reuses `$`/`API`/`escapeHtml`). Owns the fit **document** and posts it to `/api/fit/compute` on every edit (stateless recompute). Ship browser; slot grid (subsystem/high/mid/low/rig) with **module grouping** (⛓ collapses identical modules into one ×N row; ammo + state propagate to the whole group); slot-filtered item browser (restricted to what the ship can fit); **compatible-charge** picker (`/api/fit/charges`); drones / implants / boosters / cargo; **configurable skills** editor (All V / All 0 / custom + per-skill overrides); ship render + module/ammo icons + per-slot detail line (CPU/PG, cap, cycle, optimal+falloff, tracking). Right panel toggles **Stats** (Firepower · Resistances/Defense w/ EHP·HP toggle · Recharge/Reps stable-vs-max · Nav/Targeting · Resources) and **Graphs** (Effective DPS vs distance + cap-over-time, inline SVG). **Open/Save/Delete in-game fits via ESI** (`/api/fit/esi/*`), EFT import/export, and local save/load fits. |
...
| [styles.css](renderer/styles.css) | ~1065 | Dark theme, tab styles, table layouts, progress bars, modal, auth-slot cards, quota table, contracts dashboard quota bars, Working-tab pin cards, Appraisal tab three-column price tiles + percentage chips, alliance-quota-sync row + PAT-row + push button, outstanding-payout-total panel, **Module Sorter two-pane layout (`.ms-*`)**, splash. |
...
| [pi.py](python/pi.py) | ~275 | **Planetary Interaction** profitability engine (pure, prices passed in). Loads `data/pi_data.json`; `expand_to_p0` (raw basket per unit), `chain_steps`, `available_products` (what a planet's P0 can build), `evaluate` (single-step margin + full-chain-from-free-P0 profit with per-tier POCO tax), `rank_chains`. Tested by [tests/test_pi.py](python/tests/test_pi.py). |
| [pi_layout.py](python/pi_layout.py) | ~180 | PI colony template `↔` internal model. `parse_template`/`export_template` round-trip the EVE `CmdCtrLv/Diam/Pln/L/P/R` JSON byte-exact (node `k` = pin `P[k-1]`, node 0 = command center); `from_esi_detail` converts a live ESI colony (pin_ids→indices, schematic_id→output type); `great_circle_km`; file load/save. Tested by [tests/test_pi_layout.py](python/tests/test_pi_layout.py) incl. a real-template round-trip. |
| [pyfa_engine.py](python/pyfa_engine.py) | ~450 | Headless bridge to Pyfa's vendored **`eos`** fitting engine (see `python/pyfa/`). Bootstraps eos once (wx-free; in-memory saveddata forced to a shared `StaticPool` so FastAPI worker threads share the schema), then `compute_fit(doc)` builds a transient fit, computes, extracts a rich stats dict (DPS/byType + per-weapon DPS, EHP + resists, reps stable/max, cap flow, targeting, resources, per-module detail, drones/implants/cargo, T3 mode, validation) and rolls back. Also EFT parse/render and raw-sqlite browse over `eve.db` (`list_ships`, `search_items` with slot + ship-PG/CPU filter, `compatible_charges`, `list_skills`). |
| [pyfa/](python/pyfa/) | ~132 py | **Vendored Pyfa `eos` engine** (+ Pyfa's `utils/`), LGPL/GPL — see [NOTICE-fitting.md](NOTICE-fitting.md). One wx-coupling patched out (`eos/db/migration.py`). Loaded off `sys.path` at runtime by `pyfa_engine`. |
| [data/eve.db](python/data/eve.db) | ~96 MB | Pyfa's game DB (types/attributes/effects/dogma) the fitting engine reads. Built from Pyfa's static data via [build_eve_db.py](python/build_eve_db.py); committed to the repo (bundled into installers via the `data/` datas). |
| [gen_pi_data.py](python/gen_pi_data.py) / [gen_pi_pins.py](python/gen_pi_pins.py) | ~200 / ~185 | Dev/maintenance generators (not imported by `server.py`, like `gen_mineable_materials.py`). Pull from EVE Ref → `data/pi_data.json` (68 schematics, 83 types, tier-derived P0→P4, planet→P0 from extractor pin names) and `data/pi_pins.json` (90 colony pins: CC/ECU/factory/storage/launchpad CPU-PG loads, capacities, CC-per-level budgets). |
...
| [sidecar.spec](python/sidecar.spec) | — | PyInstaller spec for the bundled binary. Lists sibling modules (`pinned`, `janice`, `auth`, `config`, `esi`, `industry`, `pyfa_engine`, …) in `hiddenimports` belt-and-braces. Bundles the whole `data/` dir (refining CSV, PI/industry datasets, **`eve.db`**) and the vendored **`pyfa/`** eos tree as data (eos is imported off `sys.path` at runtime and loads effect modules by name, so shipping the source tree is more reliable than freezing it); also collects eos's dynamic deps (`sqlalchemy` submodules incl. the sqlite dialect, `numpy`, `logbook`, `roman`, `dateutil`, `greenlet`). |
