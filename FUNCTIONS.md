...
- **Access gates (Stockpile + Indy).** Two client-side convenience filters — **not** security boundaries (the shared write PAT is the real boundary), and both **always shown in Admin view**. Stockpile: the tab lives in the Admin/Operations group and is **password-gated** — `updateStockpileTabVisibility()` shows the `[data-tab="stockpile"]` button only when `stockpileUnlocked` (hardcoded `STOCKPILE_PASSWORD`, entered via the Config-tab unlock controls (`setStockpileUnlocked`), remembered in `localStorage`; the input has no `name` so it never serializes into config save/export). `refreshStockpileAccess()` now only reads `stockpile_allow_push` from config. The paste/save editor (`updateStockpileEditorVisibility()`) is gated on **both** `stockpile_allow_push` and officer-group membership (`stockpileAdminOk` — `STOCKPILE_ADMIN_GROUPS` = Industry Officer / Acquisitions Officer, set from the dashboard scrape in `refreshIndyAccess`). Indy: `INDY_GROUP_NAME = 'Industry Pilot'` (hardcoded), `indyGroupOk` (let), `updateIndyVisibility()` shows/hides the `nav-group[data-group="indy"]` (Build Planner / Fulfilment), `refreshIndyAccess()` scrapes the AA `/dashboard/` "Membership" card via `parseDashboardGroups` for an exact group-name match. Both are re-checked from `applyViewMode()`.
...
[renderer/stockpile.js](renderer/stockpile.js). Self-contained IIFE loaded after `app.js` (reuses `$`/`$$`/`API`/`escapeHtml`). Read-only dashboard of alliance stock (minerals / PI / other) synced from the shared repo, plus an admin paste/save panel (shown only to Industry Officer / Acquisitions Officer AA group members with `stockpile_allow_push` on).
